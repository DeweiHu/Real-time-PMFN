...
## Utilities
Under /utils/ there are some functions used. **util.py** includes the Nifiti file reader/writer, intensity normalizer etc. **MotionCorrection.py** is the function used for rigid registration. **label fusion** is joint label fusion sofware. **self-fusion.sh** is a shell script that inplement the label fusion software.
