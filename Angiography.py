...
Nsf = Baselines.MS_UNet(gpu,nch_1).to(device)
Nsf.load_state_dict(torch.load(modelroot+'noi2sf_MSUNet.pt'))

Ndn = Baselines.MS_UNet(gpu,nch_2).to(device)
Ndn.load_state_dict(torch.load(modelroot+'1_12.pt'))

Ndn2 = Baselines.MS_UNet(gpu,1).to(device)
Ndn2.load_state_dict(torch.load(modelroot+'MS529.pt'))
...
#    print('Denoised finish, time used: {} min'.format((t2-t1)/60))
    ...
    t1 = time.time() 
    V = MSUN_main(raw,FrameNum,Ndn2,False)
    t2 = time.time()
    print('Denoised finish, time used: {} min'.format((t2-t1)/60))
